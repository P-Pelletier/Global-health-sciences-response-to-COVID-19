...
setwd("G:/Github/Global-health-sciences-response-to-COVID-19/Moritz")

# load functions #
source("src/aux_fcts.R")
source("src/netw_fcts.R")

# define paths
inputPath <- "data/"

# read data
sample <- get_country_panel(inputPath=inputPath)

get_netw_growth <- function(CRR){
 
  ts <- c(201901:201912,202001:202012,202101:202112)
  res_mat <- matrix(NA,nrow=length(ts),ncol=2)
  rownames(res_mat) <- c(paste0(19,"/",1:12),paste0(20,"/",1:12),paste0(21,"/",1:12))
...
netw_g_CRR <- get_netw_growth(CRR=TRUE)
netw_g_nonCRR <- get_netw_growth(CRR=FALSE)
