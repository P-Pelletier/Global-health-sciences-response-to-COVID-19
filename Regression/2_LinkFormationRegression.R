...
SETTINGS <- list(
  ...
  vars_sum_absdiff = c("CoronaPubsPreCovid","nonCoronaPubsPreCovid", "total_deaths", "hdi", "gdp_pc"), # log(sum(x,y)), log(abs(x-y)) , ".sum",".absdiff"
  vars_log = c("CoronaPubsPreCovid.xy","nonCoronaPubsPreCovid.xy","geodist"), # log(a+1) #keep name
  
  
  # for regression
  remove_china_us = FALSE,
  t= c(202001:202012,202101:202112,202201:202212), # 202007:202012, #c(202002:202012,202101:202104) # 202101:202104, #202007:202012, # c(202002:202012,202101:202104),  # c(202002:202012), #202101:202104, # c(202002:202012,202104),
  months <- c(202001:202012,202101:202112,202201:202212),
  vars=list(
    depvar="coronaPubsCum.xy",
    countvars=c("CoronaPubsPreCovid.xy","nonCoronaPubsPreCovid.xy",
                ...
                "total_deaths.sum", "total_deaths.absdiff", "border_closure.mult", "lockdown.mult",
                 "gdp_pc.sum","gdp_pc.absdiff","geodist", "sameRegion","hdi.sum","hdi.absdiff"), # ,"hdi.sum","hdi.absdiff","geodist", "sameRegion"
    zerovars=c("geodist","sameRegion","nonCoronaPubsPreCovid.sum", "nonCoronaPubsPreCovid.absdiff","CoronaPubsPreCovid.sum", "CoronaPubsPreCovid.absdiff")
  ),
...
source("G:/Github/Global-health-sciences-response-to-COVID-19/Utils/netw_fcts.R")
...
add_qap <- function(B,res_set,dyads_t){
  ...
  for(i in 1:length(vars)){
    ...
    # Perform the parallel computation using foreach and %dopar%
    z_values[, i] <- foreach(j = 1:SETTINGS$B, .combine = c) %dopar% {
      library(pscl)  
      do_one_permutation(j, adj = adj, dyads_t = dyads_t, res_set = res_set, 
                         var = vars[i], count_var = count_var)
    }
    stopCluster(cl)
    # calculate p-value
...
preCovid <- panel %>%
  ...
  summarize(CoronaPubsPreCovid = sum(coronaPubs),
            nonCoronaPubsPreCovid = sum(nonCoronaPubs))
...
linksPreCovid <- panel %>%
  ...
  summarise(coronaPubsPreCovid.xy = sum(weight_corona),
            nonCoronaPubsPreCovid.xy = sum(weight_noncorona),
            dep_iPreCovid = sum(dep_i),
            dep_jPreCovid = sum(dep_j),
            paper_fundedPreCovid = sum(paper_funded),
            dep_ratio = abs(dep_iPreCovid-dep_jPreCovid)/paper_fundedPreCovid,
            .groups="drop")
...
dyads <- full_join(x=panel1,y=panel1,by="month") %>%
  ...
  mutate(CoronaPubs.xy=ifelse(is.na(coronaPubs.xy),0,coronaPubs.xy),
         ...
         dep_ratio=ifelse(is.na(dep_ratio),0,dep_ratio)) %>%
  group_by(countryCode.x,countryCode.y) %>%
  mutate(coronaPubsCum.xy = cumsum(coronaPubs.xy),
         coronaPubsCum.xy=ifelse(is.na(coronaPubsCum.xy),0,coronaPubsCum.xy)) %>%
  ungroup()
...
dyads <- dyads %>% 
  mutate(geodist = geodist::geodist(x=x,y=y,paired=TRUE, measure="geodesic") / 1000,
         ...
         CoronaPubsPreCovid.xy = log(CoronaPubsPreCovid.xy+1),
         CoronaPubsPreCovid.sum = sum_xy(CoronaPubsPreCovid.x,CoronaPubsPreCovid.y),
         CoronaPubsPreCovid.absdiff = absdiff_xy(CoronaPubsPreCovid.x,CoronaPubsPreCovid.y),
         CoronaPubsPreCovid.mult = mult_xy(CoronaPubsPreCovid.x,CoronaPubsPreCovid.y),
         border_closure.mult = mult_xy(border_cum.x,border_cum.y),
         lockdown.mult = mult_xy(lockdown_cum.x,lockdown_cum.y),
...
unistats <- dyads %>%
  ungroup() %>%
  summarise(across(.cols=all_of(c(SETTINGS$vars$countvars,SETTINGS$vars$depvar)),
...
rownames(stats_df) <- c("$closed_{i,t'}$","c_{ij,t'}", "c_{i,t0}","$c_{ij,t0}$",
                        "$Dep_{ij,t0}$","$|gdp_i - gdp_j|$ ","$gdp_i + gdp_j$", "$Distance$",
                        "$|hdi_i - hdi_j|$", "$hdi_i + hdi_j$",
                        "$locked_{i,t'}$",  "n_{i,t0}", "$n_{ij,t0}$",
                        "$Same region$", "$|death_i - death_j|$", "$death_i + death_j$" )
...
stargazer(stats_df, 
          ...
          type = "text" ,
          multicolumn = FALSE,
          out = paste0(SETTINGS$outputPath,"/Link_formation_output_stats.tex")         
)
...
for(i in 1:length(SETTINGS$t)){
  ...
  dyads_t <- dyads %>% 
    filter(month==SETTINGS$t[i] &
             nodeId.x < nodeId.y) %>% 
    as.data.frame()
  
  if(SETTINGS$remove_china_us == TRUE){
  # remove CHN
...
var_labels <- c("coronaPubsPreCovid.xy" = "$c_{ij,t0}$","nonCoronaPubsPreCovid.xy "="$n_{ij,t0}$",
                "coronaPubsPreCovid.mult" = "$c_{i,t0}$", "nonCoronaPubsPreCovid.mult" = "$n_{i,t0}$",
                "dep_ratio" = "$Dep_{ij,t0}$",
                "total_deaths.sum" = "$death_i + death_j$", "total_deaths.absdiff" = "$|death_i - death_j|$",
                "border_closure.mult" = "$closed_{i,t'}$", "lockdown.mult" = "$locked_{i,t'}$",
                "gdp_pc.sum" = "$gdp_i + gdp_j$", 'gdp_pc.absdiff' = "$|gdp_i - gdp_j|$ ",
                "geodist" = "$Distance$", "sameRegion"= "$Same region$",
                "hdi.sum" = "$hdi_i + hdi_j$", 'hdi.absdiff' = "$|hdi_i - hdi_j|$")

# Omit the desired statistics
omit_stats <- c("max.rsq", "ll", "wald", "lr","logrank","res.dev","f","ser","n")

months_clean <- parse_date_time(as.character(SETTINGS$t), orders = "ym")   # Convert to date
...
stargazer::stargazer(
  ...
  out = paste0(SETTINGS$outputPath,"/Link_formation_output_early.tex")
)
...
stargazer::stargazer(
  ...
  title = "", omit.stat = omit_stats,
  out = paste0(SETTINGS$outputPath,"/Link_formation_output_mid.tex")
)
...
stargazer::stargazer(
  ...
  out = paste0(SETTINGS$outputPath,"/Link_formation_output_end.tex")
)
...
res_lst2 <- lapply(models,extract_result)
