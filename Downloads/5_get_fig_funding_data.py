...
periods = list(grants_country.keys())
...
time_period = list(set([i for i in list(set(time_period)) if int(i)>=201901 and int(i)<=202212]))
time_period.sort()
...
grant_global = {i:[] for i in  time_period if int(i)>=201701 and int(i)<= 202212}

for grant in tqdm.tqdm(grants_info):
    for month in grants_info[grant]:
        if int(month) >= 201701 and int(month) <= 202212:
            grant_global[month].append(grant)



grant_corona = {i:[] for i in  time_period if int(i)>=201701 and int(i)<= 202212}
...
periods = [i for i in time_period if int(i)>=201701 and int(i) <= 202212]                
...
# N-project in T corona/global:

grants_info_project = {str(i):[] for i in periods if int(i)>=201701 and int(i) <= 202212}    
grants_info_project_corona = {str(i):[] for i in periods if int(i)>=201701 and int(i) <= 202212}


for grant in tqdm.tqdm(grants_info):
    for period in grants_info[grant]:
        if period in grants_info_project:
            grants_info_project[period].append(grant)

for grant in tqdm.tqdm(grants_info_corona):
    for period in grants_info_corona[grant]:
        if period in grants_info_project:
            grants_info_project_corona[period].append(grant)

grants_info_n_project = {i:len(set(grants_info_project[i])) for i in grants_info_project}
grants_info_n_project_corona = {i:len(set(grants_info_project_corona[i])) for i in grants_info_project}
    

df_fig = pd.DataFrame(index = list(grants_info_n_project))
df_fig['n_project_corona'] = df_fig.index.map(grants_info_n_project_corona)
df_fig['n_project'] = df_fig.index.map(grants_info_n_project)


# grant coming from global

grants_info_project_cumulated = {str(i):[] for i in periods if int(i)>=201701 and int(i) <= 202212}    
grants_info_project_corona_cumulated = {str(i):[] for i in periods if int(i)>=201701 and int(i) <= 202212}

i = 0
for period in tqdm.tqdm(grants_info_n_project):
    if i == 0:
        grants_info_project_cumulated[period] = grants_info_project[period]
        period_temp = period
        i += 1
    else:
        grants_info_project_cumulated[period] = list(set(grants_info_project[period] + grants_info_project_cumulated[period_temp]))
        period_temp = period
        
i = 0
for period in tqdm.tqdm(grants_info_n_project_corona):
    if i == 0:
        grants_info_project_corona_cumulated[period] = grants_info_project_corona[period]
        period_temp = period
        i += 1
    else:
        grants_info_project_corona_cumulated[period] = list(set(grants_info_project_corona[period] + grants_info_project_corona_cumulated[period_temp]))
        period_temp = period

grants_info_project_corona_new = {str(i):0 for i in periods if int(i)>=201701 and int(i) <= 202212}
grants_info_project_corona_from_global = {str(i):[] for i in periods if int(i)>=201701 and int(i) <= 202212}
grants_info_project_global_new = {str(i):0 for i in periods if int(i)>=201701 and int(i) <= 202212}
grants_info_project_global_new_future_use_covid = {str(i):0 for i in periods if int(i)>=201701 and int(i) <= 202212}


# Flexibility of researcher

i = 0
for period in tqdm.tqdm(grants_info_project_corona):
    if i == 0:
        period_temp = period
        i += 1
    else:
        for grant in list(set(grants_info_project_corona[period])):
            if grant in grants_info_project_cumulated[period_temp]:
                if grant not in grants_info_project_corona_cumulated[period_temp]:
                    grants_info_project_corona_from_global[period].append(grant)
        period_temp = period


grants_info_project_corona_from_global = {i:len(list(set(grants_info_project_corona_from_global[i]))) for i in grants_info_project_corona_from_global}

# Flexibility of institution/country for corona
i = 0
for period in tqdm.tqdm(grants_info_project_corona_new):
    if i == 0:
        grants_info_project_corona_new[period] = len(grants_info_project_corona_cumulated[period])
        period_temp = period
        i += 1
    else:
        grants_info_project_corona_new[period] = len(list(set(grants_info_project_corona_cumulated[period]) - 
                                                      set(grants_info_project_corona_cumulated[period_temp])-
                                                      set(grants_info_project_cumulated[period_temp])))
        period_temp = period
     
# Flexibility of institution/country for corona    
i = 0
for period in tqdm.tqdm(grants_info_project_global_new):
    if i == 0:
        grants_info_project_global_new[period] = len(grants_info_project_cumulated[period])
        grants_info_project_global_new_future_use_covid[period] = len(list(set(grants_info_project_cumulated[period]).intersection(set(grants_info_project_corona_cumulated["202212"]))))
        period_temp = period
        i += 1
    else:
        grants_info_project_global_new[period] = len(list(set(grants_info_project_cumulated[period]) - 
                                                      set(grants_info_project_corona_cumulated[period_temp])-
                                                      set(grants_info_project_cumulated[period_temp])))
        grants_info_project_global_new_future_use_covid[period] = len(list(set(list(set(grants_info_project_cumulated[period]) - 
                                                      set(grants_info_project_corona_cumulated[period_temp])-
                                                      set(grants_info_project_cumulated[period_temp]))).intersection(set(grants_info_project_corona_cumulated["202212"]))))
        
        period_temp = period

df_fig['new_corona_project'] = df_fig.index.map(grants_info_project_corona_new)
df_fig['new_global_project'] = df_fig.index.map(grants_info_project_global_new)
df_fig['new_global_project_future_use_covid'] = df_fig.index.map(grants_info_project_global_new_future_use_covid)
df_fig['new_corona_project_from_global'] = df_fig.index.map(grants_info_project_corona_from_global)
df_fig['share_common_grant'] = df_fig.index.map(share_dict)
df_fig["month"] = df_fig.index
df_fig.to_csv("Data/fig_funding_new.csv",index=False)
