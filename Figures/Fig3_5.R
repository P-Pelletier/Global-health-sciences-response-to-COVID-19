...
get_community <- function(g){
  ...
  coms <- tibble(countryCode=V(g)$countryCode,membership=fc$membership) %>%
    arrange(membership)
...
get_community_plot <- function(g, gamma, title="Community"){
  ...
  g_noncrr <- get_acc_network(sample=sample, covid=FALSE, start=201901, end=201912) 
...
get_cor_plot <- function(res_mat){
  ...
  if(nrow(res_mat) > 24){
    xbreaks <- c(1,7,13,19,25,31,37,43)
    xlabels <- c("Jan 2019","July 2019", "Jan 2020","July 2020","Jan 2021", "July 2021","Jan 2022","July 2022")
  }else{
...
get_qzr_plot_DEPRICATED <- function(res_mat){
  ...
  cor_lineplot <- ggplot(res_mat) +
    ...
    scale_x_discrete(limits=as.character(1:43),breaks=xbreaks, 
                     labels = x_labels) +
    scale_colour_manual("", 
                        breaks = c("z_q", "z_lambda"),
                        values = c("z_q"="blue", "z_lambda"="red"),
...
save(spring_communities, file=paste0(SETTINGS$outputPath,"plots/spring_communities.RData"))
load(paste0(SETTINGS$outputPath,"plots/spring_communities.RData"))
...
outputFile <- paste0(SETTINGS$outputPath,"/plots/Fig3.pdf")
...
plots <- list()
# line plots
plots[["lambda_q_plot"]] <- get_lambda_q_plot(res_mat)
plots[["cor_plot"]] <- get_cor_plot(res_mat)

# community, hierarchy plots for different months
g <- get_acc_network(sample=sample, covid=TRUE, start=201901, end=201912) 
v_idx <- V(g)
names(v_idx) <- V(g)$countryCode
g <- set.vertex.attribute(g, "cluster", v_idx[countryClusters[,"countryCode"]], countryClusters[,"cluster"])
...
g <- get_acc_network(sample=sample, covid=TRUE, start=201901, end=202002) 
v_idx <- V(g)
names(v_idx) <- V(g)$countryCode
g <- set.vertex.attribute(g, "cluster", v_idx[countryClusters[,"countryCode"]], countryClusters[,"cluster"])
...
g <- get_acc_network(sample=sample, covid=TRUE, start=201901, end=202012) 
v_idx <- V(g)
names(v_idx) <- V(g)$countryCode
g <- set.vertex.attribute(g, "cluster", v_idx[countryClusters[,"countryCode"]], countryClusters[,"cluster"])
...
# write out plot
outputFile <- paste0(SETTINGS$outputPath,"/plots/Fig4.pdf")
...
# write out plot
outputFile <- paste0(SETTINGS$outputPath,"/pltos/Fig5.pdf")
pdf(file=outputFile, width=pdf_width, height=pdf_height, family="Helvetica")
all_plots
dev.off()
