...
class Create_net:
    ...
    def init_network(self):
        ...
        self.dependance_funding = {key: df.copy() for key in self.time_period}
        self.no_grant = {key: df.copy() for key in self.time_period}
        self.grant = {key: df.copy() for key in self.time_period}
            ...
            if int(date) <= self.last_date and int(date) >= self.start_date:
                ...
                for i in cooc:
                    ...
                    if countries_funding:
                        self.grant[date].at[i[0], i[1]] += 1
                        if i[0] in countries_funding and i[1] not in countries_funding: 
                            self.dependance_funding[date].at[i[0], i[1]] += 1
                    else:
